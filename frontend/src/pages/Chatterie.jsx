...
function Chatterie() {
  ...
  const [globalSearchText, setGlobalSearchText] = useState(localStorage.getItem('chatterieSearchValue') || '');
  ...
  const handleGlobalSearch = (value) => {
    setGlobalSearchText(value);
    localStorage.setItem('chatterieSearchValue', value);
  };
  ...
  useEffect(() => {
    const storedValue = localStorage.getItem('chatterieSearchValue') || '';
    ...
    const fetchAnimals = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Filtrer pour ne montrer que les animaux avec statut "refuge"
          const refugeAnimals = data
            .filter(
              (animal) =>
          ...
          setFilteredAnimals(refugeAnimals);
        } else {
  ...
  const isMainPage = location.pathname === "/chatterie";
        ...
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Gestion des animaux de la chatterie (statut refuge)
            </p>
          </div>
